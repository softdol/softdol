...
public class D09_RegexEmail {
	...
	/*
	 * 	1. emails 문자열에 있는 모든 이메일들을 ArraysList에 저장해 보세요
	 * 
	 *  2. 어떤 도메인(chosun.com, )이 몇개씩 있는지 세어서 출력해 보세요
	 *  
	 *  3. 모든 도메인들을 개수 기준 내림차순 정렬하여 출력해보세요
	 */
	
	public static void main(String[] args) {
		
		String[] lineArr = emails.split("\\r\\n");
		ArrayList<String> list = new ArrayList<>();
		HashMap<String, Integer> domainList = new HashMap<>();
		for(String s : lineArr) {
			String[] tabList = s.split("\t");
			String[] domain = tabList[3].split("@");			
			list.add(tabList[3]);
			if(tabList[3].indexOf("@") > 0 && domain.length > 1) {

				String temp = domain[1].replace(";","");
				temp = temp.replace("&nbsp;","");
				Integer cnt = domainList.putIfAbsent(temp, 1);
				if(cnt != null) {
					domainList.put(temp, cnt+1);
				}
			}	
		}
		HashSet<String> keys = new HashSet<>(domainList.keySet());
		ArrayList<DomainInfo> domainInfoList = new ArrayList<>(); 
		for(String key : keys) {
			DomainInfo domainInfo = new DomainInfo(key, domainList.get(key));
			domainInfoList.add(domainInfo);
		}
		
		Comparator<DomainInfo> comparator = DomainInfo.getComparator();
		Collections.sort(domainInfoList,comparator);
...
class DomainInfo{
	...
	public DomainInfo(String domain, int domainCount) {
		this.domain = domain;
	...
	public static Comparator<DomainInfo> getComparator(){
		
		return new Comparator<DomainInfo>() {
			@Override
			public int compare(DomainInfo o1, DomainInfo o2) {
				return o2.domainCount - o1.domainCount;
			}
		};
	}
